...
import { SEVEN_WEEK_PLAN_V1 } from "./fixtures/programme-plans-v1";
import { SECTION_PACKS_V1 } from "./fixtures/section-pack-v1";

/* ------------------------------------------------------------------ */
/*  Demo Contract — approved 7-week plan with two sections             */
/*                                                                     */
/*  Drive the real UI through approval, then prove the schedule's      */
/*  order and states are backend-owned: advance virtual time through   */
/*  lecture 1 and its section, refresh, and re-verify — nothing is     */
/*  assumed from local state. The network layer (the backend) is       */
/*  intercepted; every page component and its real logic runs.         */
/* ------------------------------------------------------------------ */
 ...
 * their own lecture (6b's SECTION_PACKS_V1 weeks 1 and 5), every state
 * derived from the current virtual time exactly like the server does.
 */
...
async function assertScheduleOrderAndStates(page: import("@playwright/test").Page) {
...
test.describe("Demo Contract — 7-week plan with two sections", () => {
